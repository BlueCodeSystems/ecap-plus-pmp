...
import { Search, FileText, Filter, AlertCircle } from "lucide-react";
...
const VcaServicesDashboard = () => {
  ...
  const districts = useMemo(() => {
    if (!vcaListQuery.data) return [];
    const dists = new Set<string>();
    vcaListQuery.data.forEach((v: any) => {
      if (v.district) dists.add(v.district);
    });
  ...
  return (
    <DashboardLayout subtitle="Nationwide Service Delivery Tracking">
      <PageIntro
        eyebrow="VCA Services"
        title="Nationwide Service Delivery Tracking"
        description="Monitor child service workflow outcomes, assessments, and case plans across all 23 districts."
        actions={
      ...
      <GlowCard className="overflow-hidden">
        <div className="p-6 border-b border-slate-100">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <h3 className="text-lg font-semibold text-slate-900">VCA Service History</h3>
        ...
        <div className="overflow-x-auto">
          <Table>
            <TableHeader className="bg-slate-50/50">
              <TableRow>
                <TableHead className="font-semibold text-slate-700 w-[80px] hidden sm:table-cell">VCA ID</TableHead>
                  ...
                  <TableRow key={`${index}-${service.id}`}>
                    <TableCell
                      ...
                      onClick={() => {
                        ...
                        navigate(`/profile/vca-profile/${encodeURIComponent(String(id))}`);
                      }}
                    >
                      <span className="text-xs">{pickValue(service, ["vca_id", "vcaid", "child_id", "unique_id"])}</span>
                    </TableCell>
                    ...
                    <TableCell className="align-top px-2 sm:px-4">
                      <div className="flex flex-col">
                        <div className="flex items-center gap-2 sm:hidden">
                          ...
                          >
                            {pickValue(service, ["vca_id", "vcaid", "child_id", "unique_id"])}
                          </span>
                        </div>
