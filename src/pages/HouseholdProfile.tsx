...
import { getHouseholdsByDistrict, getHouseholdArchivedRegister, DEFAULT_DISTRICT, getCaregiverServicesByDistrict, getCaregiverReferralsByMonth } from "@/lib/api";
import { useAuth } from "@/context/AuthContext";
import DashboardLayout from "@/components/dashboard/DashboardLayout";
import GlowCard from "@/components/aceternity/GlowCard";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { ArrowLeft, User, MapPin, Calendar, ClipboardCheck, Briefcase, Layers, ShieldCheck, HeartPulse, FileText, Activity, Link2 } from "lucide-react";
...
const HouseholdProfile = () => {
  ...
  const householdServices = useMemo(() => {
    ...
    return allServices.filter((s: any) => {
      const vId = id.toLowerCase();
      const sId = String(s.household_id || s.householdId || s.hh_id || s.unique_id || s.id || "").toLowerCase();
      return sId === vId && !String(s.service || s.form_name || "").toLowerCase().includes("case plan");
    });
  }, [allServices, id]);

  const householdCasePlans = useMemo(() => {
    if (!allServices || !id) return [];
    return allServices.filter((s: any) => {
      const vId = id.toLowerCase();
      const sId = String(s.household_id || s.householdId || s.hh_id || s.unique_id || s.id || "").toLowerCase();
  ...
  const householdReferrals = useMemo(() => {
    if (!allReferrals || !id) return [];
    return allReferrals.filter((r: any) => {
      const vId = id.toLowerCase();
      const rId = String(r.household_id || r.householdId || r.hh_id || r.unique_id || r.id || "").toLowerCase();
      return rId === vId;
    });
  }, [allReferrals, id]);
      ...
      <div className="space-y-6 pb-12">
            ...
            <div className="flex flex-col items-center gap-6 md:flex-row md:items-center">
              <div className="relative h-24 w-24 overflow-hidden rounded-2xl border-4 border-white/10 bg-white/5 backdrop-blur-sm md:h-28 md:w-28">
                <Avatar className="h-full w-full rounded-none">
                  <AvatarImage src={`https://api.dicebear.com/7.x/initials/svg?seed=${caregiverName}&backgroundColor=f1f5f9`} />
                  <AvatarFallback className="text-2xl font-bold text-slate-900">{caregiverName[0]}</AvatarFallback>
                </Avatar>
              </div>
              <div className="text-center md:text-left">
                <Badge className="mb-2 bg-emerald-500/20 text-emerald-400 backdrop-blur-md border-0">
                  {isArchived ? "Archived Household" : "Active Household"}
                </Badge>
                <h1 className="text-3xl font-bold tracking-tight md:text-4xl">{caregiverName}</h1>
                <div className="mt-2 flex flex-wrap justify-center gap-3 text-sm text-slate-400 md:justify-start">
                  <span className="flex items-center gap-1.5"><Layers className="h-4 w-4" /> Household ID: {id}</span>
                  <span className="flex items-center gap-1.5"><MapPin className="h-4 w-4" /> {String(household.district || "N/A")}</span>
                  <span className="flex items-center gap-1.5"><Briefcase className="h-4 w-4" /> CW: {String(household.caseworker_name || "N/A")}</span>
                </div>
              </div>
            </div>
          ...
          <GlowCard className="lg:col-span-2">
            <div className="p-6">
              ...
              <div className="grid gap-6 sm:grid-cols-2">
                <InfoItem label="Primary Caregiver" value={caregiverName} icon={<User className="h-3.5 w-3.5" />} />
                <InfoItem label="Case Worker Name" value={String(household.caseworker_name || household.cwac_member_name || "N/A")} icon={<Briefcase className="h-3.5 w-3.5" />} />
                <InfoItem label="Facility" value={String(household.facility || "N/A")} icon={<HeartPulse className="h-3.5 w-3.5" />} />
                <InfoItem label="Ward" value={String(household.ward || "N/A")} />
                <InfoItem label="Province" value={String(household.province || "N/A")} />
                <div className="sm:col-span-2">
                  <InfoItem label="Home Address" value={String(household.homeaddress || "N/A")} icon={<MapPin className="h-3.5 w-3.5" />} />
                </div>
              </div>
            </div>
          </GlowCard>
              ...
              <div className="flex flex-col gap-3">
                {Object.entries(household).some(([key, value]) => (value === "1" || value === "true" || value === 1 || value === true)) ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-10 text-center text-slate-400">
                    ...
                    <p className="text-xs">No active markers for this household</p>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <InsightCard
            ...
            value={household.screened === "1" || household.screened === true ? "Screened" : "Pending"}
            subtitle="Current DQA screening status"
            color="emerald"
          />
          <InsightCard
            title="Last Activity"
            value={String(household.last_service_date || "N/A")}
            subtitle="Most recent service or update recorded"
            color="blue"
          />
        </div>
            ...
            <ScrollArea className="h-[400px]">
              <TabsContent value="services" className="m-0 p-0">
                <ActivityTable
                  ...
                  type="service"
                  emptyMessage="No service history found for this household."
                />
              </TabsContent>
              <TabsContent value="case-plans" className="m-0 p-0">
                <ActivityTable
                  data={householdCasePlans}
                  isLoading={isLoadingServices}
                  type="case-plan"
                  emptyMessage="No case plans found for this household."
                />
              </TabsContent>
              <TabsContent value="referrals" className="m-0 p-0">
                <ActivityTable
                  data={householdReferrals}
                  isLoading={isLoadingReferrals}
                  type="referral"
                  emptyMessage="No referral records found for this household."
                />
              </TabsContent>
            </ScrollArea>
          </Tabs>
  ...
  if (data.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center px-4">
        ...
        <p className="text-xs text-slate-400 mt-1 max-w-xs mx-auto">Try checking other categories or ensuring the Household ID is correctly registered.</p>
      </div>
    );
  }
          ...
          <TableRow key={idx} className="hover:bg-slate-50/50">
            ...
            <TableCell className="py-4 pr-6">
              <div className="flex flex-wrap items-center gap-2">
                <Badge variant="outline" className="bg-white border-emerald-200 text-emerald-700 font-bold text-[10px] uppercase px-2 py-0.5">
