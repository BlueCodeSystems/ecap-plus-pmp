...
import {
  getPmtctChildRegisterByDistrict,
  getPmtctMotherRegisterByDistrict,
  clearApiCache,
} from "@/lib/api";
...
const PMTCTRegister = () => {
  ...
  const isLoading =
    ...
    motherQueries.some((q) => q.isLoading);

  const rows = useMemo(() => {
    const infants = childQueries
      .flatMap((q) => (q.data ?? []) as Array<Record<string, unknown>>)
      .map((r) => ({
        id: pickPmctctId(r),
        householdId: pickValue(r, ["ca_id", "child_id", "unique_id"]),
        hivStatus: pickValue(r, [
          "test_result_at_birth",
          "infant_hiv_status",
          "hiv_status",
        ]),
        ...
        date: pickValue(r, [
          "dbs_at_birth_actual_date",
          "dbs_at_birth_due_date",
          "date_tested",
        ]),
        raw: r,
      }));
    const mothers = motherQueries
      .flatMap((q) => (q.data ?? []) as Array<Record<string, unknown>>)
      .map((r) => ({
        id: pickPmctctId(r),
  ...
  const stats = useMemo(() => {
    ...
    const hivByType = [
      {
        ...
        positive: rows.filter(
          (r) => r.type === "Mother" && /positive|reactive/i.test(r.hivStatus),
        ).length,
        negative: rows.filter(
          (r) =>
            r.type === "Mother" && /negative|non-reactive/i.test(r.hivStatus),
        ).length,
        unknown: rows.filter(
          (r) => r.type === "Mother" && r.hivStatus === "N/A",
        ).length,
      },
      {
        type: "Infants (HEI)",
        positive: rows.filter(
          (r) =>
            r.type === "Infant (HEI)" && /positive|reactive/i.test(r.hivStatus),
        ).length,
        negative: rows.filter(
          (r) =>
            r.type === "Infant (HEI)" &&
            /negative|non-reactive/i.test(r.hivStatus),
        ).length,
        unknown: rows.filter(
          (r) => r.type === "Infant (HEI)" && r.hivStatus === "N/A",
        ).length,
      },
    ];
    const facilityMap = new Map<string, number>();
    rows.forEach((r) =>
      facilityMap.set(r.facility, (facilityMap.get(r.facility) ?? 0) + 1),
    );
    const byFacility = Array.from(facilityMap.entries())
      ...
      .map(([facility, count]) => ({
        facility:
          facility.length > 18 ? `${facility.slice(0, 16)}...` : facility,
        count,
      }));
    ...
    const enrolmentTrend = Array.from(monthMap.entries())
      .sort((a, b) => a[0].localeCompare(b[0]))
      .slice(-6)
      .map(([month, count]) => {
        const [y, m] = month.split("-");
        return {
          month: new Date(Number(y), Number(m) - 1).toLocaleString("en", {
            month: "short",
            year: "2-digit",
          }),
          enrolments: count,
  ...
  const handleExport = () => {
    ...
    downloadCsv(
      `pmtct-register-${selectedDistrict.toLowerCase().replace(/\s+/g, "-")}.csv`,
      headers,
      exportRows,
    );
  };
            ...
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                ...
                >
                  ...
                  {isLoading ? "—" : stats.total.toLocaleString()} records
                </Badge>
                <Badge
                  variant="outline"
                  className="gap-1 border-slate-200 bg-white/70 text-[10px] text-slate-600"
                >
              ...
              <p className="mt-1 text-xs text-slate-600">
                Mother &amp; infant pairs, HIV exposure, and follow-up status.
              </p>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              ...
              <GlowCard className="p-4 h-[340px]">
                <div className="mb-2">
                  <div className="text-sm font-bold text-slate-900">
                    HIV Status by Beneficiary Type
                  </div>
                  <p className="text-[10px] text-slate-400 font-medium">
                    Mothers vs Infants (HEI)
                  </p>
                </div>
              ...
              <GlowCard className="p-4 h-[340px]">
                ...
                <div className="h-[280px]">
                  {isLoading ? (
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart layout="vertical" data={stats.byFacility}>
                        <CartesianGrid
                          strokeDasharray="3 3"
                          horizontal={false}
                          stroke="#f1f5f9"
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
              <GlowCard className="p-4 h-[340px]">
                <div className="mb-2">
                  ...
                  <p className="text-[10px] text-slate-400 font-medium">
                    Monthly enrolments over last 6 months
                  </p>
                </div>
                <div className="h-[280px]">
                  {isLoading ? (
                    <LoadingDots />
                  ) : stats.enrolmentTrend.length ? (
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={stats.enrolmentTrend}>
                        <defs>
                          <linearGradient
                    ...
                    <div className="h-full flex items-center justify-center text-sm text-slate-400">
                      No enrolment date data available
                    </div>
                  )}
                </div>
              </GlowCard>
            ...
            <GlowCard noHover>
              <div className="p-6 border-b border-slate-50 flex flex-col md:flex-row justify-between items-center gap-4">
                <div className="relative w-full md:w-80">
                  ...
                  <Input
                    placeholder="Search by ID, HHID or facility..."
                    className="pl-10 h-11 rounded-2xl bg-slate-50 border-transparent focus:bg-white focus:ring-[#00a67e] transition-all font-medium"
                    value={searchQuery}
                ...
                <div className="flex items-center gap-2">
                  <Button
                    ...
                    className="h-10 rounded-xl px-4 text-xs font-bold text-slate-600 border-slate-200"
                  >
                    Clear Filters
                  </Button>
                  <Button
                    ...
                    className="h-10 border-slate-200 bg-white text-slate-700 font-bold rounded-xl transition-all shadow-sm px-4 text-xs uppercase tracking-wider"
                  >
                ...
                <Table>
                  <TableHeader className="bg-gradient-to-r from-emerald-50/80 via-teal-50/60 to-sky-50/40">
                    <TableRow className="hover:bg-transparent border-b border-emerald-100/60">
                      <TableHead className="py-4 pl-8 text-[11px] font-bold uppercase tracking-wider text-emerald-800">
                        PMTCT ID
                      </TableHead>
                      <TableHead className="py-4 text-[11px] font-bold uppercase tracking-wider text-emerald-800">
                        Type
                      </TableHead>
                      <TableHead className="py-4 text-[11px] font-bold uppercase tracking-wider text-emerald-800">
                        HIV Status
                      ...
                      <TableHead className="py-4 text-right pr-8 text-[11px] font-bold uppercase tracking-wider text-emerald-800">
                        Action
                      </TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {isLoading ? (
                      ...
                      rows.map((item, i) => (
                        <TableRow
                          key={`${item.id}-${i}`}
                          className="group border-b border-emerald-50/60 transition-colors hover:bg-gradient-to-r hover:from-emerald-50/40 hover:via-teal-50/20 hover:to-transparent"
                        >
                          <TableCell className="py-4 pl-8 font-mono text-xs font-semibold text-[#00a67e]">
                            {item.id}
                          </TableCell>
                              ...
                              onClick={() => {
                                try {
                                  sessionStorage.setItem(
                                    "pmtct_profile_record",
                                    JSON.stringify(item.raw),
                                  );
                                } catch {
                                  // ignore storage failures
                                }
                                navigate("/profile/pmtct-details", {
                                  state: { record: item.raw },
                                });
                              }}
                            >
                              View
