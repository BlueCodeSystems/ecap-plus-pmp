...
const hasArtInitiation = (record: any) => {
  ...
  return (
    Boolean(value) &&
    !["n/a", "na", "none", "null", "undefined", "0", "no", "false"].includes(
      value,
    )
  );
};
...
const HTSRegister = () => {
  ...
  const lockedDistrict =
    isRestrictedToDistrict && user?.location ? user.location : null;
    ...
    return rawData
      .filter((r) => {
        ...
        if (selectedDistrict !== "All Districts") {
          const variants = discoveredDistrictsMap.get(selectedDistrict) ?? [
            selectedDistrict,
          ];
  ...
  const facilityData = useMemo(() => {
    const facilities = new Map<
      string,
      { name: string; positive: number; linked: number }
    >();
      ...
      <div className="relative mb-6 overflow-hidden rounded-3xl border border-emerald-200/60 bg-white/70 backdrop-blur-xl shadow-[0_30px_80px_-50px_rgba(15,118,110,0.55)]">
        ...
        <div className="relative z-10 flex flex-col gap-4 px-5 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-7 sm:py-6">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                ...
                {(displayStats?.total || 0).toLocaleString()} tests
              </Badge>
            ...
            <p className="mt-1 text-xs text-slate-600">
              Testing outcomes, linkage to ART, and contact tracing across the
              cohort.
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2 sm:shrink-0">
            ...
            >
              <SelectTrigger className="w-full sm:w-[180px] h-9 bg-white/80 border-slate-200 backdrop-blur-md text-xs">
                <div className="flex items-center gap-2">
                  ...
                  <SelectValue
                    placeholder={
                      htsQuery.isLoading ? "Loading…" : "Select District"
                    }
                  />
                </div>
              </SelectTrigger>
            ...
            >
              <RefreshCcw
                className={`h-3.5 w-3.5 ${htsQuery.isFetching ? "animate-spin" : ""}`}
              />
              Sync
            </button>
          </div>
            ...
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-4">
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Testing outcomes, linkage to ART, and contact tracing across
                  the cohort.
                </p>
              </div>
            </div>
                ...
                <tbody>
                  {allRecords.slice(0, 50).map((item: any, idx: number) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-sm text-slate-700">
                        {item.health_facility || item.facility || "N/A"}
                      </td>
                      <td className="px-4 py-3 text-sm text-slate-700">
                        {item.hiv_result || item.hiv_status || "N/A"}
                      </td>
                      <td className="px-4 py-3 text-sm text-slate-700">
                        {item.art_date || item.art_date_initiated || "N/A"}
                      </td>
                      <td className="px-4 py-3 text-sm text-slate-700">
