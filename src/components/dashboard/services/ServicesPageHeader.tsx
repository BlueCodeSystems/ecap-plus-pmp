import { Database, Clock, Activity, Sparkles, Home, ClipboardList, HeartPulse } from "lucide-react";
...
const PALETTES: Record<ServiceType, {
...
const ServicesPageHeader = ({ type = "household", title, subtitle, source, generatedAt }: Props) => {
  ...
  return (
    <div className="relative mb-6 overflow-hidden rounded-3xl border border-emerald-200/60 bg-white/70 backdrop-blur-xl shadow-[0_30px_80px_-50px_rgba(15,118,110,0.45)]">
      <div className="absolute inset-0" style={{ background: `radial-gradient(circle at 10% 20%, ${SHARED_PALETTE.radialA}, transparent 55%)` }} />
      <div className="absolute inset-0" style={{ background: `radial-gradient(circle at 90% 30%, ${SHARED_PALETTE.radialB}, transparent 45%)` }} />
      <div className={`pointer-events-none absolute -top-40 -left-32 h-[24rem] w-[24rem] rounded-full ${SHARED_PALETTE.blob1} blur-[110px] animate-pulse [animation-duration:6s]`} />
      <div className={`pointer-events-none absolute -bottom-32 right-[-6rem] h-[26rem] w-[26rem] rounded-full ${SHARED_PALETTE.blob2} blur-[120px] animate-pulse [animation-duration:8s] [animation-delay:-3s]`} />
      ...
      <div className="relative z-10 flex flex-col gap-4 px-5 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-7 sm:py-6">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <span className={`text-[11px] font-bold uppercase tracking-[0.2em] ${SHARED_PALETTE.chipText}`}>{SHARED_PALETTE.eyebrow}</span>
            <span className="text-slate-400 text-[11px]">·</span>
            <span className="text-[11px] text-slate-600">{dateStr}</span>
            <Badge variant="outline" className={`ml-1 gap-1 ${SHARED_PALETTE.chip} text-[10px]`}>
              <Activity className="h-3 w-3" /> Live
            </Badge>
          </div>
          <h1 className="mt-1 text-xl sm:text-2xl font-extrabold tracking-tight">
            <span className={`bg-gradient-to-r ${SHARED_PALETTE.gradientText} bg-clip-text text-transparent`}>
              {title}
            </span>
            <Badge variant="outline" className={`ml-2 gap-1 ${SHARED_PALETTE.chip} align-middle text-[10px] shadow-sm`}>
        ...
        {(source || generatedAt) && (
          <div className="flex flex-wrap items-center gap-2 sm:shrink-0">
            ...
            {generatedAt && (
              <Badge variant="outline" className="gap-1 border-slate-200 bg-white/70 text-[10px] text-slate-600 backdrop-blur-md">
